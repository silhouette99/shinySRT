...
fun_server_web <- function() {
  afun <- glue::glue(
    ...
    '                            ynames = NULL) {{\n',
    '  \n',
    '  if(length(intersect(x, meta_group$group)) == 0){{\n',
    '    meta$all_cell <- \'all_cell\'\n',
    '    x = \'all_cell\'\n',
    '  }}\n',
    '  \n',
    '  groups = NULL\n',
    ...
    '  names(cols) <- clu\n',
    '  \n',
    '  if (length(which(names(genesets) %in% c(g1, g2))) >= 2) {{\n',
    ...
    '    h5file$close_all()\n',
    '  }} else{{\n',
    '    shiny::validate(\'Incorrect number of genes or mismatched gene names! \')\n',
    '  }}\n',
    '  \n',
    '  if (length(intersect(meta$sampleID, highlight)) == 0 |\n',
    '      length(intersect(meta$sampleID, highlight)) == length(meta$sampleID)) {{\n',
    '    meta <- meta\n',
    '  }} else{{\n',
    '    meta <- meta[which(meta$sampleID %in% highlight), ]\n',
    '  }}\n',
    ...
    '    # gg_scatter\n',
    '  }}\n',
    'coexp_score_spatial <- function(g1,\n',
    '                                g2,\n',
    '                                input_image,\n',
    '                                meta,\n',
    '                                meta_slice,\n',
    ...
    '    meta$val2 <- val2\n',
    '    \n',
    '    val1[val1 <= max(val1) * threshold1] <- 0\n',
    '    val2[val2 <= max(val2) * threshold2] <- 0\n',
    '    \n',
    '    # val1 <- val1/max(val1)\n',
    '    # val2 <- val2/max(val2)\n',
    '    \n',
    '    # meta[, \'Coexp_Score\'] <- sqrt(val1 * val2)\n',
    '    meta[, \'Coexp_Score\'] <- val1 * val2\n',
    '    \n',
    '    # li <- (max(val1) * threshold1)*(max(val2) * threshold2)\n',
    '    \n',
    '    h5file$close_all()\n',
    ...
    '  \n',
    '  meta_slice <-\n',
    '    meta[which(meta$sampleID %in% meta_slice$sampleID),]\n',
    '  \n',
    '  meta_slice$tooltip <-\n',
    '    paste0(\n',
    '      \'sampleID: \',\n',
    ...
    '      \'\\n\',\n',
    '      \'Coexp_score:\',\n',
    '      round(meta_slice[[\'Coexp_Score\']], 2)\n',
    '    )\n',
    '  \n',
    '  \n',
    '  if (!is.null(input_image)) {{\n',
    ...
    '  }}\n',
    '  \n',
    '  if (class(input_image) == \'array\' |\n',
    '      class(input_image) == \'raster\') {{\n',
    ...
    '      data_id = \'sampleID\'\n',
    '    )\n',
    '  ) + bg + xlim(xrange) + ylim(rev(yrange)) + theme_classic() +\n',
    '    theme(\n',
    '      axis.line = element_blank(),\n',
    '      axis.text = element_blank(),\n',
    '      axis.ticks = element_blank(),\n',
...
sp_server_p2 <- function(df_select){
  ...
  cosc <- lapply(1:length(df_select$slice), function(x){
    glue::glue("input$coexpscore{x}_selected")
  }) %>% unlist() %>% paste(collapse = ',')
  p2_gtab <- glue::glue(
    'output$coexp_leg <- renderPlot({{coexleg(gene1 = input$gene_f1,gene2 = input$gene_f2,inpcol = input$col_sel)}},height = 250)\n',
    'output$coexp_vln <- ggiraph::renderGirafe({{\n',
    ...
    '    g1 = input$gene_f1,\n',
    '    g2 = input$gene_f2,\n',
    '    highlight = unique(c(sps,{cosp},{cosc})),\n',
    '    threshold1 = input$thrd1,threshold2 = input$thrd2,\n',
    '    meta_group = meta_group_add$meta,\n',
    '    meta = add_meta$meta,\n',
    '    data = data,\n',
    '    genesets = genesets)\n',
  ...
  p2slice <- lapply(1:length(df_select$slice), function(x){
    glue::glue(
      ...
      '  session$sendCustomMessage(type = \'coexpscore{x}_set\', message = character(0))\n',
      ...
      '             genesets = genesets,threshold1 = input$thrd1,inpcol = input$col_sel,threshold2 = input$thrd2))}})\n',
      '## co-gene score\n',
      'output$coexpscore{x} <- ggiraph::renderGirafe({{\n',
      '  sp <- coexp_score_spatial(\n',
      '    meta_slice = add_meta{x}(),\n',
      '    meta = add_meta$meta,\n',
      '    data = data,\n',
      '    g1 = input$gene_f1,\n',
      ...
      '  inter_spatial(gg_scatter = sp)}})\n',
      '## dowload plot\n',
      'output$coexpscore{x}.pdf <- downloadHandler(\n',
      '  filename = function() {{paste0(input$gene_f1, \"_\", input$gene_f2, \"_\", {x}, \".pdf\")}},\n',
      '  content = function(file) {{\n',
      '    ggsave(\n',
      '      file,\n',
      '      device = \"pdf\",\n',
      '      height = input$coexpscore{x}.h,\n',
      '      width = input$coexpscore{x}.w,\n',
      '      plot = coexp_score_spatial(\n',
      '        meta_slice = add_meta{x}(),\n',
      '        meta = add_meta$meta,\n',
      '        data = data,\n',
      '        g1 = input$gene_f1,\n',
      ...
      '        threshold2 = input$thrd2\n',
      '      ))}})\n',
      'output$coexpscore{x}.png <- downloadHandler(\n',
      '  filename = function() {{paste0(input$gene_f1, \"_\", input$gene_f2, \"_\", {x}, \".png\")}},\n',
      '  content = function(file) {{\n',
      '    ggsave(\n',
      '      file,\n',
