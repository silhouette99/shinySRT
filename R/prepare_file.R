...
preparedata_shinyspatial <- function(dat,
  ...
  mat_exp <- hdf5r::H5File$new(filename, mode = "w")
...
#' Prepare data files required for shiny app(web)
#'
#' This function could generate data files for shiny app by saptial transcriptomic data, e.g. matrix, metadata, coordination, H&E images.
#'
#' Generate data files required for shiny app.Six files will be generated,
...
#'
#'
#' @import data.table reticulate dplyr Seurat SummarizedExperiment scran scatterpie
#'
#'
#'
#'
#' @export
preparedata_web <- function(dat,
                            meta.to.include = NA,
                            tooltip_col = NULL,
                            maxlevel = 50,
                            shiny.dir = 'shinyspatial_app',
  ...
  dmExist = TRUE # dim matrix
  col_box <- c(
    '#ff3030','#1e90ff','#ffd700','#ff6eb4','#bf3eff','#c1ffc1','#00ffff','#ff1493',
    '#7fff00','#ff0000','#0000ff','#00ff00','#ffc125','#ffb5c5','#9b30ff','#54ff9f','#87ceff','#ffaeb9',
    '#c0ff3e','#ff4040','#836fff','#00ff7f','#ffff00','#ff3e96','#ff83fa','#9aff9a',
    '#4876ff','#ff3e96','#00f5ff','#8b1a1a','#104e8b','#008b00','#8b7500','#8b3a62',
    '#68228b','#698b69','#008b8b','#8b0a50','#458b00','#eedfcc','#76eec6','#eec591',
    '#dbe9e9','#8ee5ee','#ee6a50','#eee8cd','#eead0e','#bcee68','#ee7600','#8deeee',
    '#00b2ee','#ee6363','#eeeee0','#eee685','#b2dfee','#eedc82','#ee00ee','#ee30a7','#006400'
  )
  ...
  if (class(dat)[1] == "Seurat") {
    ...
    if (class(dat@images[[1]])[[1]] == 'VisiumV1') {
      ## visium
      image <- dat@images
      ### seurat spot coordination are needed to flip
      coordi <- lapply(names(image), function(x) {
    ...
    ima <- dat[['image']]
    slice_samples <-
      lapply(colnames(dat[['metadata']]), function(x) {
        ms <- dat[['metadata']] %>% data.table::data.table()
        unique_col <- unique(ms[[x]])
        if (length(intersect(unique_col, names(ima))) == length(names(ima))) {
          samples <- x
    ...
    coordi <- lapply(names(coordi), function(x) {
      sub_coordi <- coordi[[x]]
      if (length(which(colnames(sub_coordi) %in% c('imagerow', 'imagecol'))) < 2) {
        colnames(sub_coordi)[c(
          grep(pattern = 'x|X|col|Col', colnames(sub_coordi)),
          grep(pattern = 'y|Y|row|Row', colnames(sub_coordi))
        )] <- c('imagecol', 'imagerow')
      }
      sub_coordi[, slice_samples[1]] <- x
      return(sub_coordi)
  ...
  coordi <- dplyr::distinct(coordi, .keep_all = T)
  ...
  if (gene.mapping[1] == TRUE) {
    if (sum(grepl("^ENSG000", gex.rownm)) >= sum(grepl("^ENSMUSG000", gex.rownm))) {
      tmp1 = read.table(system.file("extdata", 'hg_map.txt.gz', package = 'shinySRT'),
                        header = T)
      # tmp1 = data.table::fread(system.file("extdata",'hg_map.txt.gz',package = 'shinySRT'))
    } else {
      tmp1 <-
        read.table(system.file("extdata", 'mm_map.txt.gz', package = 'shinySRT'),
                   header = T)
      # tmp1 = data.table::fread(system.file("extdata",'mm_map.txt.gz',package = 'shinySRT'))
    }
    gene.mapping = tmp1$GeneName
  ...
  if (!is.null(ima[[1]])) {
    ...
    boxsize <-
      c(
        max(coordi$imagerow) - min(coordi$imagerow),
        max(coordi$imagecol) - min(coordi$imagecol)
      )
  }
  ...
  if (class(dat)[1] == "Seurat") {
    ...
    da <- as(as.matrix(dat[['data']]), "dgCMatrix") 
  }
  
  if (!is.null(ima[[1]])) {
    ranges <- lapply(ima, function(x) {
      list(xrange = c(0, dim(x)[2]),
           yrange = c(0, dim(x)[1]))
    })
  } else{
    ranges <- lapply(ima, function(x) {
  ...
  saveRDS(da, paste0(shiny.dir, '/data.Rds'))
  saveRDS(coordi, paste0(shiny.dir, '/meta.Rds'))
  saveRDS(meta_group, paste0(shiny.dir, '/meta_group.Rds'))
  saveRDS(genesets, paste0(shiny.dir, '/genesets.Rds'))
  saveRDS(df_select, paste0(shiny.dir, '/df_select.Rds'))
